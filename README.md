...
O projeto conta com uma suíte de testes robusta cobrindo as camadas críticas do sistema.
### 📊 Backend (NestJS)
Testes unitários cobrindo **Services**, **Controllers** e **Lógica de Negócio** (Auth, Weather, Export, Users).
**Como executar:**
\`\`\`bash
cd backend
npm run test        # Executar todos os testes
npm run test:watch  # Executar em modo watch (desenvolvimento)
npm run test:cov    # Gerar relatório de cobertura
\`\`\`
**Cobertura Atual (~80% na lógica principal):**
- **AuthService**: Autenticação Local, Google OAuth, Recuperação de Senha.
- **WeatherService**: CRUD de logs, estatísticas, paginação.
- **ExportService**: Geração de CSV e XLSX.
- **Controllers**: Validação de rotas e integração com services.
